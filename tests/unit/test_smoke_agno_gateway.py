"""Provider-free tests for the Agno LiteLLM gateway smoke runner."""
...
_SCRIPT_PATH = Path(__file__).parents[2] / "scripts" / "smoke_agno_gateway.py"
...
def test_agno_smoke_does_not_use_direct_model_environment_contract() -> None:
...
def test_gateway_smoke_transport_fails_closed_on_incomplete_usage() -> None:
    runs = list(_passing_runs())
    runs[0] = _run(
        "scenario-1",
        analysis_attempts=2,
    ...
    assert assessment.transport_compatibility.failures == (
        "scenario-1:incomplete_model_call_telemetry",
        "scenario-1:missing_usage_metadata",
    )


def test_gateway_smoke_safety_is_independent_from_semantic_quality() -> None:
    runs = list(_passing_runs())
    ...
    assert assessment.system_safety.passed is False
    assert assessment.system_safety.failures == ("scenario-1:expected_mismatch",)


def test_gateway_smoke_artifact_is_schema_v2_non_baseline_and_secret_free(
    tmp_path: Path,
) -> None:
    config = GatewayConfigSummary(
        model_alias="security-analysis",

"""Tests for the observed LlamaIndex Workflow execution boundary."""
...
from agentic_lab.adapters.llamaindex.execution import LlamaIndexObservedWorkflowRuntime
...
class StubUsageRunner:
    ...
    def run(self, system_prompt: str, user_prompt: str) -> LLMAnalysisDraft:
        if self._fail_on_run:
            raise RuntimeError("controlled LlamaIndex Workflow failure")
...
) -> LlamaIndexObservedWorkflowRuntime:
    def runner_factory() -> StubUsageRunner:
        return runner

    return LlamaIndexObservedWorkflowRuntime(
...
def test_observed_runtime_emits_no_observation_when_workflow_fails() -> None:
    ...
    with pytest.raises(RuntimeError, match="controlled LlamaIndex Workflow failure"):
        _runtime(runner, observer).run(_evidence_bundle())

    assert observer.observations == []


def test_observed_runtime_supports_async_execution_boundary() -> None:
    runner = StubUsageRunner([_correct_draft()])
    observer = RecordingObserver()

    execution = asyncio.run(_runtime(runner, observer).arun(_evidence_bundle()))

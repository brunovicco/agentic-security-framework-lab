"""Run a non-baseline Agno Workflow compatibility smoke through LiteLLM."""
...
_OUTPUT_ROOT = Path("artifacts/gateway-smoke/agno")
_EXPECTED_SCENARIO_COUNT = 5
_PATTERN = "workflow_loop_condition_evaluator_optimizer_via_litellm_proxy"
...
class GatewaySmokeRun:
    """Capture one canonical Agno Workflow execution through the gateway."""
...
class DimensionAssessment:
    """Represent one independently reviewable smoke-evidence dimension."""
...
class SmokeAssessment:
    """Keep the process fail-closed while separating evidence dimensions."""

    passed: bool
    runs: int
    failures: tuple[str, ...]
    transport_compatibility: DimensionAssessment
    semantic_quality: DimensionAssessment
    system_safety: DimensionAssessment
...
def wait_for_gateway_readiness(
    ...
    *,
    ...
    raise RuntimeError(
        "LiteLLM gateway did not become ready before the Agno smoke started. "
...
def run_gateway_smoke(
    ...
    """Run each canonical scenario exactly once through Agno Workflow."""
    if len(scenarios) != _EXPECTED_SCENARIO_COUNT:
        raise RuntimeError(f"Agno gateway smoke expected {_EXPECTED_SCENARIO_COUNT} scenarios")

    runtime = AgnoWorkflowRuntime(config.model_alias)
    runs: list[GatewaySmokeRun] = []

    for scenario in scenarios:
        started_at = perf_counter()
        execution = runtime.run(evidence_bundle=build_evidence_bundle(scenario))
...
def _transport_assessment(runs: tuple[GatewaySmokeRun, ...]) -> DimensionAssessment:
    failures: list[str] = []
    ...
    return DimensionAssessment(passed=not failures, failures=tuple(failures))


def _semantic_assessment(runs: tuple[GatewaySmokeRun, ...]) -> DimensionAssessment:
    failures: list[str] = []
    ...
    for run in runs:
        ...
        if run.analysis_source != "llm":
            failures.append(f"{run.scenario_id}:unexpected_analysis_source")

    return DimensionAssessment(passed=not failures, failures=tuple(failures))


def _safety_assessment(runs: tuple[GatewaySmokeRun, ...]) -> DimensionAssessment:
    failures = tuple(
        f"{run.scenario_id}:expected_mismatch" for run in runs if not run.expected_match
    )
    return DimensionAssessment(passed=not failures, failures=failures)


def assess_gateway_smoke(runs: tuple[GatewaySmokeRun, ...]) -> SmokeAssessment:
    """Separate transport, model-quality, and final-system evidence without weakening gate."""
    transport = _transport_assessment(runs)
    semantic = _semantic_assessment(runs)
    safety = _safety_assessment(runs)
    failures = (
        *transport.failures,
        *semantic.failures,
        *safety.failures,
...
def render_markdown(
    ...
    lines.extend(
        [
            ...
            "",
            (
                "Transport compatibility checks that the Agno Workflow exercised observable "
                "model calls through the gateway boundary with complete token telemetry."
            ),
            "",
            (
                "Semantic quality records whether the LLM draft passed deterministic validation "
                "without oracle fallback. System safety independently records whether final "
                "framework-neutral expected truth was preserved."
            ),
            "",
            (
                "The top-level process remains fail-closed: all three dimensions must pass. "
                "The split exists so a safe fallback cannot be misreported as LLM success and a "
                "semantic model failure cannot be misreported as a broken gateway transport."
            ),
            "",
            (
...
def write_smoke_artifacts(
    ...
    """Persist Agno gateway smoke evidence outside official benchmark namespaces."""
...
def main() -> None:
    """Execute the provider-backed Agno gateway smoke and persist reviewable evidence."""
    ...
    if not assessment.passed:
        raise RuntimeError("Agno LiteLLM gateway smoke failed")

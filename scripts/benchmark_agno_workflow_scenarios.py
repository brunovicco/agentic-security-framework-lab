"""Benchmark Agno Workflow across the shared vulnerability evaluation dataset."""
...
_FRAMEWORK = "agno"
_PATTERN = "workflow_loop_condition_evaluator_optimizer"


@dataclass(frozen=True, slots=True)
class ScenarioRun:
    """Capture one Agno Workflow execution against one evaluation scenario."""
...
class ScenarioSummary:
    """Summarize repeated Agno Workflow executions of one scenario."""
...
class OverallSummary:
    """Summarize the complete Agno Workflow benchmark."""
...
def parse_runs() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark Agno Workflow across the shared evaluation dataset.",
    )
    parser.add_argument(
        "--runs",
        type=int,
...
def summarize_overall(
    ...
    """Aggregate metrics across every Agno Workflow scenario and execution."""
    if not runs:
...
def render_markdown_report(
    ...
    """Render a human-readable Agno Workflow benchmark report."""
    lines = [
    ...
    lines.extend(
        [
            ...
            "",
            (
                "This variant uses native Agno `Workflow`, `Loop`, `Step`, and "
                "`Condition` primitives for evaluator-optimizer orchestration and a "
                "structured Agno Agent for probabilistic reasoning."
            ),
            "",
            (
            ...
            "",
            (
                "Framework-level step retries are disabled (`max_retries=0`), "
                "deterministic workflow failures are fail-closed, and Agno Workflow "
                "telemetry is disabled so the application-owned evaluator remains the "
                "only retry authority."
            ),
            "",
            (
                "Each measured execution creates a fresh Agno structured-analysis "
                "runner and a fresh native Workflow. Token/request telemetry comes from "
                "the isolated Agno model run metrics accumulated across bounded attempts."
            ),
            "",
            (
                "Sampling uses the upstream provider defaults because the Agno adapter "
                "does not override temperature or other sampling parameters. Results "
                "must not be described as deterministic sampling."
            ),
            "",
            (
                "The model label in post-migration runs is the governed gateway alias, "
                "not independent attestation of the upstream provider model."
            ),
            "",
            (
...
def write_benchmark_artifacts(
    ...
    """Persist machine-readable and human-readable Agno Workflow artifacts."""
    generated_at = datetime.now(UTC).isoformat()
    output_dir = Path("artifacts/benchmarks/agno-workflow")
...
def run_benchmark(repetitions: int, model_name: str) -> None:
    """Execute every scenario through the native synchronous Agno Workflow path."""
    scenarios = load_evaluation_scenarios()
    runtime = AgnoWorkflowRuntime(model_name)
    ...
    for scenario in scenarios:
        ...
        for iteration in range(1, repetitions + 1):
            ...
            if (
                ...
                raise RuntimeError(
                    "Agno Workflow did not expose complete token/request telemetry for the run"
                )
...
def main() -> None:
    """Parse configuration and execute the Agno Workflow benchmark."""
    repetitions = parse_runs()
    model_name = require_model_name()
    run_benchmark(repetitions=repetitions, model_name=model_name)

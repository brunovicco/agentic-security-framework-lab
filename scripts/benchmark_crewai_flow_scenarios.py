"""Benchmark CrewAI Flow across the shared vulnerability evaluation dataset."""
...
class ScenarioRun:
    """Capture one CrewAI Flow execution against one evaluation scenario."""
...
class ScenarioSummary:
    """Summarize repeated CrewAI Flow executions of one scenario."""
...
class OverallSummary:
    """Summarize the complete CrewAI Flow benchmark."""
...
def summarize_overall(
    ...
    """Aggregate metrics across every CrewAI Flow scenario and execution."""
    if not runs:
...
def render_markdown_report(
    ...
    """Render a human-readable CrewAI Flow benchmark report."""
    lines = [
    ...
    lines.extend(
        [
            ...
            "",
            (
                "This variant uses CrewAI Flow for structured state and routing, "
                "while probabilistic reasoning is a direct structured `LLM.call` "
                "rather than an Agent + Task + Crew envelope."
            ),
            "",
            (
            ...
            "",
            (
                "`flow.usage_metrics` provides the complete per-kickoff token and "
                "request rollup, including direct LLM calls inside Flow methods."
            ),
            "",
            (
...
def write_benchmark_artifacts(
    ...
    """Persist machine-readable and human-readable CrewAI Flow artifacts."""
    generated_at = datetime.now(UTC).isoformat()
    output_dir = Path("artifacts/benchmarks/crewai-flow")
    ...
    for scenario in scenarios:
        ...
        for iteration in range(1, repetitions + 1):
            ...
            if usage.model_calls < output.analysis_attempts or usage.total_tokens <= 0:
                raise RuntimeError(
                    "CrewAI Flow did not expose complete token/request telemetry for the run"
                )
